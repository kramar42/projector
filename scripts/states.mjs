...
# Every value declared here is carried by at least one note in the vault. If you add
...
/**
 * A doc a note links to — and, since every markdown file in a vault is a card, a
 * bare card in its own right. It carries no frontmatter at all, so it is also the
 * fixture's one example of derived identity: `docs/resolves.md` is titled by its
 * leading heading and identified by its filename.
 */
const DOCS = {
...
This file has no frontmatter, so it is a card the app derived a name for. Its title is the
...
// The cards sit at the root; everything the app owns is under `.projector/`.
for (const dir of ['.projector/views', 'docs']) mkdirSync(join(root, dir), { recursive: true });

writeFileSync(join(root, '.projector', 'facets.yaml'), FACETS, 'utf8');
writeFileSync(
  join(root, '.gitignore'),
  '# Derived. This whole directory is generated by scripts/states.mjs.\n.projector/*.db*\n',
  'utf8',
);

for (const note of NOTES) {
  writeFileSync(join(root, `${note.id}.md`), `---\n${frontmatter(note)}\n---\n\n${note.body}`, 'utf8');
}
for (const [name, body] of Object.entries(VIEWS)) {
  writeFileSync(join(root, '.projector', 'views', `${name}.yaml`), body, 'utf8');
}
...
console.log(`${root}
  ${NOTES.length} notes + ${Object.keys(DOCS).length} bare (docs/), ${Object.keys(VIEWS).length} views, ${declared.size} distinct facet values
