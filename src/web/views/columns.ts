 ...
 * that are not a facet on any card, so nothing in `show` could name them.
 ...
 *
 * `Cards`, `Blocked` and `Untriaged` come from `projectRollups`, which has a
 ...
 * columns that were blank for every ordinary card beside it — width spent on a
