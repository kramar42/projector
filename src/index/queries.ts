 ...
 *
 * Filtering, grouping and counting all run in memory over the note map — see
...
/**
 * Notes matching an FTS query, most relevant first.
 *
...
export function search(db: DatabaseSync, query: string, limit?: number): Row[] {
  const sql =
    `SELECT r.id, r.title, r.updated
       FROM fts JOIN notes r ON r.id = fts.id
...
export function counts(db: DatabaseSync, facets: Facets): Record<string, number> {
  ...
  return {
    ...
    projects: one('SELECT count(*) AS n FROM notes WHERE is_project = 1'),
    // A container is a note something names — derived, like the glyph, and
