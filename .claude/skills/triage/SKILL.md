---
name: triage
description: Triage cockpit cards that are missing a project, priority or status — propose values for each, present them for approval, and apply only what is approved. Use when asked to triage, sort out, clean up or organise cards, when asked "what needs sorting", or after an import has left a pile of untriaged cards. Do not use for creating new cards from external sources; that is the capture skill.
---

# Triage

Give incomplete cards a project, a priority and a status. Read the `cockpit` skill first if you have
...
## 1. Get the worklist from `ck`, not from a guess

```bash
ck untriaged --json --limit 40
```
...
```bash
ck set <id> --facet project=infra --facet priority=backlog --facet status=planning
```
...
Report the counts: how many cards changed, how many were left for the user, and any `ck check`
warning your batch introduced. If `ck check` gained an error, fix it or revert that card — never
