---
name: pj-setup
description: Set up a projector vault's channels and credentials — ask which sources this vault should sweep, verify each one actually answers, and write .projector/config.yaml. Use when a vault is new, when `pj intake` or a link says something is not configured, when asked to connect Jira / GitHub / Slack / Gmail, or when asked to check what projector can reach. Do not use to file or sort cards; those are the pj-capture and pj-triage skills.
---
...
a token in a transcript. The same rule as `pj-capture`: a secret's value never enters a card, and it
